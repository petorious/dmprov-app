...
const path='/archive/assets';
const form_name='asset';
...
class Asset extends Component {
  ...
  handleClose = () => {
    ...
    setDialogIsOpen('delete_asset', false);
  ...
  render() {
    ...
    const menuList=[
      ...
      },
      {
        ...
        onClick: ()=>{setDialogIsOpen('delete_asset', true);}
      }
    ]
    ...
    return (
      <Activity
        ...
        title={intl.formatMessage({id: match.params.uid?'edit_asset':'create_asset'})}>
        ...
        <div style={{margin: 15, display: 'flex'}}>
          ...
          <FireForm
            ...
            onSubmitSuccess={(values)=>{history.push('/archive/assets');}}
            onDelete={(values)=>{history.push('/archive/assets');}}
            uid={match.params.uid}>
            <AssetForm />
          </FireForm>
        </div>
        <Dialog
          title={intl.formatMessage({id: 'delete_asset_title'})}
          actions={actions}
          modal={false}
          open={dialogs.delete_asset===true}
          onRequestClose={this.handleClose}>
          {intl.formatMessage({id: 'delete_asset_message'})}
        </Dialog>
...
)(injectIntl(withRouter(withFirebase(muiThemeable()(Asset)))));
